...
class ISVM(SoftMarginSVM):
    def online_training(self, kernel = 'Linear', parameter = 1.0, t_size = 1000, test = True):
        if test == True:
            self.t = []
            self.x_range = []
        n_samples, _ = self.sample.shape
        iter_num = int(n_samples / t_size)
        if n_samples % t_size != 0:
            iter_num += 1
        self.kernel = kernel
        self.parameter = parameter
        if n_samples <= t_size:
        ...
        for x in range(iter_num):
            ...
            if x == 0:
                this_sample = np.copy(self.sample[:t_size])
                this_label = np.copy(self.label[:t_size])
                ...
                if x != iter_num -1:
                    this_sample = np.copy(self.sample[x*t_size:x*t_size+t_size])
                    this_label = np.copy(self.label[x*t_size:x*t_size+t_size])
                else:
                    this_sample = np.copy(self.sample[x*t_size:])
                    this_label = np.copy(self.label[x*t_size:])
                ...
                fil = a[(a_copy.argsort()[::-1][0:300])[-1]]
                print(a, fil)
                sv = a >= fil
                ind = np.arange(len(a))[sv]
                self.a = a[sv]
                ...
                if test == True:
                    self.t.append(self.testing(self.test_sample, self.test_label))
                    self.x_range.append(x)
                    print(self.t[-1])

        plt.plot(self.x_range, self.t)
        plt.xlabel("Iter")
        plt.ylabel("Accuracy")
        plt.title("SVM")
        plt.show()
...
if __name__ == "__main__":
    ...
    I = ISVM("train_data.csv", "train_label.csv")
    I.input_Margin(0.1)
